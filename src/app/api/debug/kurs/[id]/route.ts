...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const [k] = await db.select({ id: kurs.id, edukator: kurs.edukator }).from(kurs).where(eq(kurs.id, kursId));
    ...
    return NextResponse.json({ success: true, kursId, kursEdukator: k?.edukator ?? null, tokenSub });
  } catch (error: any) {
